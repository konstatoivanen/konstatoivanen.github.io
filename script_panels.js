...
function udpate_scaling()
{
	...
	var is_mobile = vw <= vh ? 1 : 0;
	var is_desktop = vw > vh ? 1 : 0;
    ...
    document.documentElement.style.setProperty('--is-mobile', is_mobile);
    document.documentElement.style.setProperty('--is-desktop', is_desktop);
}
...
function expand_tile(id, do_focus)
{
    ...
    switch (element.dataset.behavior)
    {
        ...
        case "local_image":
			if (try_update_previous_tile(id))
			{
				element.insertAdjacentHTML("afterend", "<div id='expand_div' class='tile_parent' style='animation-name:ClipIn_Left_Img'><div class='divider'></div><div class='tile_image'><img src='" + element.dataset.content + "'/></div></div>");
				break;
			}
            return;
